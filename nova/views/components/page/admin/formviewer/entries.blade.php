							...
							<div class="btn-toolbar pull-right">
								<div class="btn-group">
									@if (Sentry::getUser()->hasAccess('form.read'))
										<a href="{{ URL::to('admin/formviewer/'.$form->key.'/detail/'.$e->data_id) }}" class="btn btn-sm btn-default icn-size-16">{{ $_icons['view'] }}</a>
									@endif

									@if (Sentry::getUser()->hasAccess('form.update'))
										<a href="{{ URL::to('admin/formviewer/'.$form->key.'/edit/'.$e->data_id) }}" class="btn btn-sm btn-default icn-size-16">{{ $_icons['edit'] }}</a>
									@endif
								</div>

								@if (Sentry::getUser()->hasAccess('form.delete'))
									<div class="btn-group">
										<a href="#" class="btn btn-sm btn-danger js-entry-action icn-size-16" data-action="delete" data-key="{{ $formKey }}" data-id="{{ $e->data_id }}">{{ $_icons['remove'] }}</a>
									</div>
								@endif
							</div>
						</div>
