...
@section('content')
	<div class="card text-center">
		<div class="card-block">
			...
			@if ($env->get('php'))
				<div>
					@icon('nova/src/Setup/views/design/images/check', ['class' => 'icon text-success'])
				</div>
			@else
				<div>
					@icon('nova/src/Setup/views/design/images/warning', ['class' => 'icon text-danger'])
				</div>
				<p>Uh oh! It looks like your server isn't running a high enough version of PHP. In order to use {{ config('nova.app.name') }}, your server needs to be running at least PHP 7.0. Your server is currently running <strong class="text-danger">{{ PHP_VERSION }}</strong>. Sometimes, web hosts run multiple versions of PHP and it's possible to switch your account over to a different PHP version. Get in touch with your web host and see if this is something they can help you resolve.</p>
			@endif
		</div>
	</div>
