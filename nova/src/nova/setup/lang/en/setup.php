...
return array(
	'config' => array(
		...
		'email' => array(
			'intro' => "Like previous versions of Nova, email is a critical component to keep players updated on new content, private messages and many other things. Unlike previous versions though, we no longer recommend using PHP's built-in <code>mail()</code> function (this is how email has been sent in all previous versions of Nova). In order to provide greater reliability delivering emails to players, you should use an SMTP email service. You can read more about emails in Nova and your options for setting this up in the <a href='#' target='_blank'>email options</a> document in AnodyneDocs. It's still possible to use PHP's <code>mail()</code> function, but we don't recommend it.</p><p>Before setting up Nova 3, the email config file needs to be created. In order to do that, you'll need to have the following information about how you want to send email from Nova:</p><ol><li>Whether you want to use SMTP or PHP's <code>mail()</code> function</li><li>The name of the SMTP server you're using</li><li>The server port</li><li>The encryption type</li><li>The email address and name you want emails to come from</li><li>Your SMTP server username</li><li>Your SMTP server password</li></ol><p>If you've chosen to use PHP's <code>mail()</code> function, you won't need anything, but if you've elected to use SMTP email, then in all likelihood, your web host or SMTP host (if you're using a third-party service) gave you this information when you created your account. If you don't have this information, contact your web host/SMTP host and they'll be able to get you what you need in order to continue.</p><blockquote><p>If for any reason Nova can't automatically create the email config file for you, you'll be able to do so manually. If you want to do this manually, you can open <code>nova/src/nova/setup/generators/mail.php</code>, copy its contents and paste them into a new file called <code>mail.php</code> in the <code>app/config/:env</code> directory.</p></blockquote>",
