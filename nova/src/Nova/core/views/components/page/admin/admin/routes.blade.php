...
<div class="tab-content">
	<div id="routesUser" class="tab-pane active">
		@if (isset($routes['user']))
			<input type="text" id="searchUserRoutes" class="input-small alt-col-4" placeholder="{{ lang('Short.search', langConcat('User_created Routes')) }}">

			<table class="table table-striped" id="userRoutes">
				<thead>
					<tr>
						<th class="alt-col-4">{{ lang('uri') }}</th>
						<th class="alt-col-1">{{ lang('Verb') }}</th>
						<th class="alt-col-5">{{ lang('Resource') }}</th>
						<th class="alt-col-2"></th>
					</tr>
				</thead>
				<tbody>
				@foreach ($routes['user'] as $route)
					<tr>
						<td class="alt-col-4">{{ $route->uri }}</td>
						<td class="alt-col-1">{{ strtoupper($route->verb) }}</td>
						<td class="alt-col-5">{{ $route->resource }}</td>
						<td class="alt-col-2">
	...
	<div id="routesCore" class="tab-pane">
		@if (isset($routes['core']))
			<input type="text" id="searchCoreRoutes" class="input-small alt-col-4" placeholder="{{ lang('Short.search', langConcat('Core Routes')) }}">
			...
			<table class="table table-striped" id="coreRoutes">
				<thead>
					<tr>
						<th class="alt-col-4">{{ lang('uri') }}</th>
						<th class="alt-col-1">{{ lang('Verb') }}</th>
						<th class="alt-col-6">{{ lang('Resource') }}</th>
						<th class="alt-col-1"></th>
					</tr>
				</thead>
				<tbody>
				@foreach ($routes['core'] as $route)
					<tr>
						<td class="alt-col-4">{{ $route->uri }}</td>
						<td class="alt-col-1">{{ strtoupper($route->verb) }}</td>
						<td class="alt-col-6">{{ $route->resource }}</td>
						<td class="alt-col-1">
							@if (Sentry::getUser()->hasAccess('routes.create'))
