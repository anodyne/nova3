    ...
    <div>
        <span class="relative flex w-full rounded-md shadow-sm">
            <button
                @click="open = !open"
                type="button"
                class="flex items-center justify-between cursor-default relative w-full rounded-md ring-1 ring-gray-200 dark:ring-gray-200/[15%] focus-within:ring-2 focus-within:ring-primary-400 dark:focus-within:ring-primary-400 bg-gray-50 dark:bg-gray-700/50 py-2 px-3 text-left focus:outline-none focus:bg-white dark:focus:bg-gray-800 transition leading-normal space-x-4"
                aria-haspopup="true"
                aria-expanded="true"
                :aria-expanded="open"
            >
                <span class="flex items-center space-x-2 truncate">
                    @isset($selected)
                        @icon($selected, 'h-6 w-6 shrink-0')
                        <span>{{ $selected }}</span>
                    @else
                        Select an icon
                    @endisset
                ...
                @else
                    <div class="py-4">
                        <div class="text-center leading-normal">
                            ...
                            <div class="text-gray-900 dark:text-gray-100 font-medium mt-1">&lsquo;{{ $search }}&rsquo;</div>
                        </div>
                    </div>
                @endif
            </div>
