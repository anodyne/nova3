<div x-data="{ open: false }" x-on:rank-groups-dropdown-close.window="open = false" x-on:keydown.window.escape="open = false" x-on:click.away="open = false" class="relative inline-block w-full text-left leading-0">
        ...
        <div class="relative z-10 max-h-60 divide-y divide-gray-100 overflow-auto rounded-lg bg-white ring-1 ring-gray-900/5 dark:divide-gray-700 dark:bg-gray-700 dark:ring-gray-700 dark:highlight-white/10">
            <div class="p-1.5" role="menu" aria-orientation="vertical" aria-labelledby="options-menu">
                <div class="group mb-4 flex items-center space-x-3 rounded-md border-2 border-gray-100 bg-gray-100 px-2 py-2 text-gray-500 focus-within:border-gray-300 focus-within:bg-white focus-within:text-gray-700 dark:border-gray-600 dark:bg-gray-600 dark:text-gray-400 dark:focus-within:border-gray-500 dark:focus-within:bg-gray-700">
                    ...
                    <input wire:model.debounce.250ms="search" type="text" placeholder="Find a rank group..." class="flex w-full appearance-none border-none bg-transparent p-0 focus:outline-none focus:ring-0 dark:placeholder-gray-400" />
                ...
                @forelse ($filteredGroups as $group)
                    <button wire:click="selectGroup({{ $group->id }})" type="button" class="group flex w-full items-center justify-between rounded-md px-4 py-2 text-base font-medium text-gray-500 transition hover:bg-gray-100 hover:text-gray-900 focus:outline-none dark:text-gray-400 dark:hover:bg-gray-600/50 dark:hover:text-gray-100 md:text-sm" role="menuitem">
                        ...
                        @if ($selectedId === $group->id)
                            <span x-description="Checkmark, only display for selected option." x-state:on="Highlighted" x-state:off="Not Highlighted" :class="{ 'text-white': selected === {{ $group->id }}, 'text-primary-500': !(selected === {{ $group->id }}) }" class="flex items-center text-primary-500">
                ...
                @empty
                    <div class="flex flex-col items-center pb-6 pt-2">
                        ...
                        <x-button.outline wire:click="createAndSelectGroup" type="button" color="primary">Create this group</x-button.outline>
                    </div>
                @endforelse
            </div>
        </div>
