<x-panel x-bind="parent" class="{{ $reordering ? 'overflow-hidden' : '' }}" x-data="filtersPanel()">
    <x-panel.header
        title="Rank items"
        message="Combine the rank group, rank name, and rank images to define your game's ranks"
        :border="$reordering"
    >
        @if (! $reordering)
            <x-slot name="actions">
                ...
                @can('create', $rankItemClass)
                    <x-button.filled :href="route('ranks.items.create')" color="primary" leading="add">
                        Add
                    </x-button.filled>
                @endcan
            </x-slot>
        @else
            <x-slot name="message">
                <x-panel.primary icon="arrows-sort" title="Change sorting order" class="mt-4">
                    <div class="space-y-4">
                        <p>
                            Rank items will appear in the order below whenever they're shown throughout Nova. To change
                            the sorting of rank items, drag them to the desired order. Click Finish to return to the
                            management view.
                        </p>
        ...
        @if ($rankItemCount > 0)
            ...
            @forelse ($rankItems as $rankItem)
                <x-table-list.row wire:sortable.item="{{ $rankItem->id }}" wire:key="item-{{ $rankItem->id }}">
                    ...
                    >
                        <x-badge :color="$rankItem->status->color()">
                            {{ $rankItem->status->displayName() }}
                        </x-badge>
                    </div>
                        ...
                        <x-slot name="actions">
                            <x-dropdown placement="bottom-end">
                                ...
                                <x-dropdown.group>
                                    @can('view', $rankItem)
                                        <x-dropdown.item
                                            :href="route('ranks.items.show', $rankItem)"
                                            icon="show"
                                            data-cy="view"
                                        >
                                            <span>View</span>
                                    ...
                                    @can('update', $rankItem)
                                        <x-dropdown.item
                                            :href="route('ranks.items.edit', $rankItem)"
                                            icon="edit"
                                            data-cy="edit"
                                        >
                                            <span>Edit</span>
                                ...
                                @can('delete', $rankItem)
                                    <x-dropdown.group>
                                        <x-dropdown.item-danger
                                            type="button"
                                            icon="trash"
                                            x-on:click="$dispatch('dropdown-toggle');$dispatch('modal-load', {{ json_encode($rankItem) }});"
                                            data-cy="delete"
                                        >
                                            <span>Delete</span>
                                        </x-dropdown.item-danger>
            ...
            @empty
                <x-slot name="emptyMessage">
                    ...
                    >
                        <x-slot name="primary">
                            <x-button.filled :href="route('ranks.items.create')" color="primary">
                                Add a rank item
                            </x-button.filled>
                        </x-slot>
