    ...
    <x-dropdown trigger-color="blue-soft" trigger-size="xs">
        ...
        <x-dropdown.group class="px-2 py-2">
            <div class="group flex items-center rounded-md bg-gray-100 border-2 border-gray-100 text-gray-600 px-2 py-2 space-x-3 focus-within:border-gray-200 focus-within:bg-gray-1 focus-within:text-gray-700">
                ...
                <input wire:model.debounce.250ms="search" type="text" placeholder="Find a role..." class="flex w-full appearance-none bg-transparent border-none p-0 focus:ring-0 focus:outline-none">
        ...
        @isset ($results)
            <x-dropdown.group>
                @forelse ($results as $item)
                    <x-dropdown.item type="button" wire:click="addRole({{ $item['id'] }}, {{ $item }})">
                        {{ $item['display_name'] }}
                    </x-dropdown.item>
                @empty
                    <x-dropdown.text>
