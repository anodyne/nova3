                        ...
                        @if (filled($search))
                            <div class="absolute z-10 mt-2 w-full rounded-md bg-white p-1.5 shadow-lg ring-1 ring-gray-900/5 dark:bg-gray-800">
                                @forelse ($filteredCharacters as $character)
                                    <x-dropdown.item type="button" class="group flex w-full items-center rounded-md px-4 py-2 text-base font-medium text-gray-700 transition hover:bg-gray-50 focus:outline-none dark:text-gray-300 dark:hover:bg-gray-600/50 md:text-sm" wire:click="assignCharacter({{ $character->id }})">
    ...
    @if ($characters->count() > 0)
        <div @class([
            ...
            @foreach ($characters as $character)
                <div class="flex items-center justify-between bg-white px-6 py-3 last:rounded-b-lg dark:bg-gray-900" wire:key="row-{{ $character->id }}">
                    <div class="truncate">
                    ...
                    <div class="flex items-center justify-end space-x-3">
                        ...
                        @else
                            @can('update', $user)
                                <x-button.outline tag="button" color="gray" size="xs" wire:click="assignPrimaryCharacter({{ $character->id }})">Make primary</x-button.outline>
                            @else
                                <x-badge :color="$character->type->color()">{{ $character->type->displayName() }}</x-badge>
                            @endcan
                        @endif
                            ...
                            <x-dropdown placement="bottom-end">
                                ...
                                <x-dropdown.group>
                                    <x-dropdown.text>
                                        ...
                                        <strong class="font-semibold text-gray-700 dark:text-gray-200">{{ $character->displayName }}</strong>
                                        from {{ $user->name }}?
                                    </x-dropdown.text>
                                </x-dropdown.group>
                                <x-dropdown.group>
                                    <x-dropdown.item-danger type="button" icon="trash" wire:click="unassignCharacter({{ $character->id }})">Remove</x-dropdown.item-danger>
