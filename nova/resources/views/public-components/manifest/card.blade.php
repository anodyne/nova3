...
>
    ...
    <div
        @class([
            'nv-manifest-card-content mt-4 flex flex-col',
            'items-center' => $orientation === 'center',
        ])
    >
        <div
            class="nv-manifest-card-content-name flex items-center text-lg/7 font-bold tracking-tight text-gray-900 dark:text-white"
        >
            @if (filled($character))
        ...
        <div class="nv-manifest-card-content-position text-sm/6 text-gray-600 dark:text-gray-400">
            @if (filled($character))
        ...
        @if (filled($character) && in_array('rank-image', $options))
            <div class="nv-manifest-card-content-rank mt-4">
                <x-rank :rank="$character?->rank"></x-rank>
            </div>
        @endif

        <div class="nv-manifest-card-content-badges mt-1">
            @if (filled($character))
