...
<th
    {{ $attributes->merge(['class' => 'px-6 py-3 bg-gray-50 dark:bg-gray-700/40'])->only('class') }}
>
    @unless ($sortable)
        <span class="flex text-left text-sm font-medium text-gray-500 dark:text-gray-400">{{ $slot }}</span>
    @else
        <button {{ $attributes->except('class') }} class="flex items-center space-x-1 text-left text-sm font-medium text-gray-500 dark:text-gray-400 group focus:outline-none focus:underline">
            <span>{{ $slot }}</span>
