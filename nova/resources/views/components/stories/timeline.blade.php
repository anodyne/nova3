		...
		<div class="grid grid-cols-12 items-baseline relative p-3 sm:p-5 overflow-hidden" wire:key="story-{{ $story->id }}">
			...
			<div class="col-start-2 col-span-11 ml-0">
				...
				<div class="relative flex flex-col md:flex-row md:items-center space-y-3 md:space-y-0 md:space-x-8 mt-3 text-base md:text-sm">
					...
					<span class="font-medium text-gray-500 dark:text-gray-400">{{ number_format((int) $story->posts_count) }} @choice('post|posts', $story->posts_count)</span>

					@if ($story->children->count() > 0)
						<span class="font-medium text-gray-500 dark:text-gray-400">{{ number_format((int) $story->recursive_posts_count) }} posts in all stories</span>
					@endif
