...
@section('content')
    <x-page-header>
        <x-slot name="title">{{ request()->has('pending') ? 'Pending' : '' }}Themes</x-slot>
        ...
        <x-slot name="actions">
            ...
            @can('create', 'Nova\Themes\Models\Theme')
                <x-button.filled :href="route('themes.create')" color="primary">Add Theme</x-button.filled>
            @endcan
        </x-slot>
    </x-page-header>

    @if ($themes->count() > 0)
        <div class="mx-auto mt-12 grid w-full max-w-2xl gap-6 lg:max-w-none lg:grid-cols-3">
    ...
    <div class="mx-auto mt-16 w-full max-w-2xl">
        <x-panel.primary icon="info">
            <div class="flex-1 md:flex md:justify-between">
                <p class="text-base md:text-sm">Looking for more themes? Check out the Nova Exchange!</p>
