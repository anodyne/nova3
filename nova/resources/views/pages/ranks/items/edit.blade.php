...
@section('content')
    ...
    <x-panel x-data="{ tab: 'base', base: '{{ $item->base_image }}', overlay: '{{ $item->overlay_image }}' }">
        <x-form :action="route('ranks.items.update', $item)" method="PUT">
            ...
            <x-form.section title="Rank Info" message="You can build up your rank with a few clicks. Assign it to a group, set a name, and pick your base and overlay images to build your rank quickly and easily.">
                <x-input.group label="Rank Group" for="group_id" :error="$errors->first('group_id')">
                    <select name="group_id" id="group_id" class="form-select w-full | sm:w-2/3">
                        @foreach ($groups as $group)
                            <option value="{{ $group->id }}"  @if($item->group_id === $group->id) selected @endif>
                                {{ $group->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <x-input.group label="Rank Name" for="name_id" :error="$errors->first('name_id')">
                    <select name="name_id" id="name_id" class="form-select w-full | sm:w-2/3">
                        @foreach ($names as $name)
                            <option value="{{ $name->id }}" @if($item->name_id === $name->id) selected @endif>
                                {{ $name->name }}
                            </option>
                        @endforeach
                    </select>
