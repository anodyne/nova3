                ...
                <x-tab.panel name="details" class="space-y-12">
                    <x-fieldset>
                        <x-fieldset.group constrained>
                            <x-input label="Name" name="name" :value="old('name', $postType->name)" />

                            <x-input label="Key" name="key" :value="old('key', $postType->key)" />
                            ...
                            <x-input
                                ...
                                type="color"
                                name="color"
                                :value="old('color', $postType->color)"
                            />
                    ...
                    <x-switch
                        label="Active"
                        name="status"
                        :checked="old('status', $postType->status === BasicStatus::Active)"
                        align="left"
                    />
                </x-tab.panel>
                        ...
                        <x-fieldset.group constrained>
                            @foreach (PostTypeField::cases() as $field)
                                <x-panel
                                    ...
                                    x-data="{
                                        expanded: false,
                                        enabled: {{ Js::from(old('fields.' . $field->value . '.enabled', $postType->fields->{$field->value}->enabled)) }},
                                        required: {{ Js::from(old('fields.' . $field->value . '.required', $postType->fields->{$field->value}->required)) }}
                                    }"
                                >
                    ...
                    <x-fieldset>
                        ...
                        <x-fieldset.group constrained>
                            <x-switch
                                ...
                                :checked="old('options[notifiesUsers]', $postType->options->notifiesUsers)"
                            />

                            <x-switch
                                label="Include in post tracking stats"
                                name="options[includedInPostTracking]"
                                :checked="old('options[includedInPostTracking]', $postType->options->includedInPostTracking)"
                            />

                            <x-switch
                                label="Allow multiple authors"
                                name="options[allowsMultipleAuthors]"
                                :checked="old('options[allowsMultipleAuthors]', $postType->options->allowsMultipleAuthors)"
                            />

                            <x-switch
                                label="Allow characters as authors"
                                name="options[allowsCharacterAuthors]"
                                :checked="old('options[allowsCharacterAuthors]', $postType->options->allowsCharacterAuthors)"
                            />

                            <x-switch
                                label="Allow users as authors"
                                name="options[allowsUserAuthors]"
                                :checked="old('options[allowsUserAuthors]', $postType->options->allowsUserAuthors)"
                            />
                            ...
                            <x-switch
                                ...
                                :checked="old('options[showContentInTimelineView]', $postType->options->showContentInTimelineView)"
                            />
