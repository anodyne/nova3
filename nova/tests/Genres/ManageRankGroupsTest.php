...
class ManageRankGroupsTest extends DatabaseTestCase
{
	...
	public function unauthorized_users_cannot_manage_rank_groups()
	{
		...
		$this->get(route('ranks.groups.edit', 1))->assertRedirect(route('login'));
		$this->patch(route('ranks.groups.update', 1))->assertRedirect(route('login'));
		$this->delete(route('ranks.groups.destroy', 1))->assertRedirect(route('login'));
		...
		$this->get(route('ranks.groups.edit', 1))->assertStatus(403);
		$this->patch(route('ranks.groups.update', 1))->assertStatus(403);
		$this->delete(route('ranks.groups.destroy', 1))->assertStatus(403);
	}
	...
	public function a_rank_group_can_be_created()
	{
		...
		$this->post(route('ranks.groups.store'), $position->toArray());
	...
	public function all_rank_groups_can_be_updated()
	{
		...
		$this->patch(route('ranks.groups.update'), $positionsData);
	...
	public function a_rank_group_can_be_deleted()
	{
		...
		$this->delete(route('ranks.groups.destroy', [$this->position]));
